...
- 板块六：设计协作（第 14-15 篇，HarWork 独有的设计模式）
- 板块七：DevOps（第 16-19 篇，从 cloc 到 canary 全套）

**跳读路径**（按你最关心的层切入）：
- 关心 **断线续传**？→ 第 12 篇（板块五）
- 关心 **rm -rf 怎么拦**？→ 第 10 篇（板块四）
- 关心 **怎么让社区扩展工具**？→ 第 07 篇（板块三）
- 关心 **怎么部署到生产**？→ 第 17 篇（板块七）

> 注意 **层 ≠ 篇**：16 层栈是技术依赖视角，19 篇是阅读体验视角。同一层可能拆成 2-3 篇（如安全层 = 静态分析 + 路径守卫 + 审批流，3 篇），同一篇可能横跨多层（如设计协作篇横跨 Hook + Skill + UI 三层）。所以"先看全景图，再决定怎么读"比"按编号死读"更高效。
